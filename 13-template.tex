...
The main challenge will be modelling the communication between the controller and the different devices, which can vary in functioning.
...
is heavily inspired by REST applications, but we decided to let each device implement its own state. This respects the 
single responsibilty principle. This is also more explicit, allowing each device to define its own fields.
...
Problem: The logger has to be extensible, in order to log additional information.
For example, we might want to log timestamps, or we might not want to because of file sizes.
This could potentially be done dnynamically.
Solution: used the Decorator design pattern, which allows to wrap dnynamically objects at runtime,
and overload their functionalities.
...
\begin{itemize}
	...
	This resulted in no raw null values checking.

...
I built the Electrical Meter as part of the controller in the MVC pattern. Its main role is to retreive the state of the Outlet objects and calculate the istant power absorption and the global electrical consumption.
...
The state of the Outlet, represented by the OutletState class, is basically the consumed power over time calculated in Watt-hours (Wh) from the device plugged into the Outlet.
I decided not to let the Outlet know wether there's a connected device or not, I thought it made sense for the device to update its own power consumption and set that in the Outlet.

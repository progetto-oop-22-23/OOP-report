...
When we first ideated HOMER, we all agreed on building an Electrical Meter which let the user check the electrical consumption of the various devices during the simulation.
Its main role is to retreive the state of the Outlets and calculate the istant power absorption and the global electrical consumption.
It also implements an automatic safety check on the consumption, which cuts the power to the most consuming Outlet when the global power absorption reaches the limit of 4kW.
The Electrical Meter acts as part of the Controller in the MVC pattern. It's a DiscreteObject, which updates its state in the updateTick method, retrieves the state of the Outlets,
performs the safety checks and elaborates the Power absorption and electrical consumption, then it updates the View through the view manager.
The view manager is the class that implements the methods and keeps the references to the elements of the View for the FXML loader, it is called the controller class in the FXML standard.

% UML of Electrical Meter here.
...
%Arguably the minimum consumption might always be considered 0.0, but I wanted to leave out more flexibility for possible future scenarios, like Outlets that can never be completely shut down, even in emergency cases as an electrical overload.
The Outlet has no "knowledge" wether there's a plugged Device or not, I thought it was better to keep it this way, and let the device set its own power consumption and "send" it to the Outlet, basically asking for power when it needs it, emulating a real-case scenario, 
...
PoweredDevice contains the needed methods to control the Device's consumption, retreive the PoweredDeviceInfo associated with it, and set a new Outlet as the one its plugged.
...
%UML of PoweredDevice and PoweredDeviceInfo here.
